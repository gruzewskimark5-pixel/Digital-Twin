...
import { AlertTriangle, ShieldAlert, Satellite, Terminal, CheckSquare } from 'lucide-react';
...
interface AnomalyPanelProps {
  anomalies: Anomaly[];
  injectAnomaly: () => void;
  executePlaybook: (id: string) => void;
}

// ⚡ Bolt Optimization: Wrap component in React.memo to prevent unnecessary re-renders
// during the 1-second simulation tick. By passing stable `useCallback` functions from App.tsx,
// this complex DOM structure is only reconciled when the `anomalies` array actually changes.
export const AnomalyPanel = memo(function AnomalyPanel({ anomalies, injectAnomaly, executePlaybook }: AnomalyPanelProps) {
  return (
