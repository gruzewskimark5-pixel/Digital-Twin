...
type TabType = 'TWIN' | 'H_LAYER' | 'I_LAYER' | 'J_LAYER' | 'K_LAYER' | 'L_LAYER' | 'M_LAYER' | 'N_LAYER' | 'O_LAYER' | 'P_LAYER' | 'Q_LAYER' | 'R_LAYER' | 'S_LAYER' | 'T_LAYER' | 'U_LAYER' | 'V_LAYER' | 'W_LAYER' | 'X_LAYER' | 'Y_LAYER' | 'Z_LAYER';
...
// ⚡ Bolt Optimization: Extract and memoize the highly repetitive navigation tabs.
// Wrapping this component in React.memo() prevents 20+ string-parsing operations
// (twMerge/clsx) from running on every single 1-second simulation tick, saving CPU cycles.
export const NavigationTabs = memo(({ activeTab, setActiveTab }: { activeTab: TabType, setActiveTab: (tab: TabType) => void }) => {
  return (
