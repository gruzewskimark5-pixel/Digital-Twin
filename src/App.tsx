...
type LayerType = typeof LAYERS[number] | 'AA_LAYER' | 'AB_LAYER' | 'AC_LAYER' | 'AD_LAYER';

const LayerTabs = React.memo(({ activeTab, setActiveTab }: { activeTab: LayerType, setActiveTab: (tab: LayerType) => void }) => {
  return (
    <>
      {LAYERS.map(layer => (
        <button
          ...
          className={cn(
            ...
            activeTab === layer ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/50" : "text-gray-500 hover:text-gray-300"
          )}
        >
          {layer.replace('_', '-')}
        </button>
...
export default function App() {
  const [activeTab, setActiveTab] = useState<LayerType>('Z_LAYER');
    ...
    <div className="min-h-screen p-4 md:p-6 flex flex-col gap-6">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 glass-panel p-4 rounded-xl">
        ...
        <div className="flex items-center gap-2 bg-black/40 p-1 rounded-lg border border-gray-800 overflow-x-auto max-w-full">
          <LayerTabs activeTab={activeTab} setActiveTab={setActiveTab} />
        </div>
