            ...
            <div className="mt-auto">
              ...
              <div className="bg-black/60 rounded-lg p-3 border border-gray-800 h-48 overflow-hidden font-mono text-[10px] leading-relaxed flex flex-col gap-1">
                {telemetryLog.map((log, i) => (
                  // ⚡ Bolt Optimization: Use `log` string as stable key instead of index `i`.
                  // Because logs are prepended every second, using `key={i}` forces React to mutate the text
                  // content of every single DOM element on every tick.
                  // Using `key={log}` allows React to just insert 1 new element and remove the last,
