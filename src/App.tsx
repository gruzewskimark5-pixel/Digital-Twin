...
export const LAYER_TABS = [
  ...
  'AA_LAYER', 'AB_LAYER', 'AC_LAYER', 'AD_LAYER'
] as const;

export type LayerTab = typeof LAYER_TABS[number];

// ⚡ Bolt Optimization: Memoize the massive list of tabs
// In a heavily-ticking application (1s intervals), rendering 24+ tabs
// causes continuous execution of twMerge/clsx which are expensive string operations.
// Wrapping this in React.memo() ensures the tabs only re-render when the activeTab changes.
const TabNavigation = React.memo(({
  activeTab,
  setActiveTab
}: {
  activeTab: LayerTab,
  setActiveTab: (tab: LayerTab) => void
}) => {
  return (
